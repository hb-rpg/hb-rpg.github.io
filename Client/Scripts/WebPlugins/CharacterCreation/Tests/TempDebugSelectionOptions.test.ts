...
import {
    updateRaceItemsData, updateRaceLanguageData, updateRaceEdgesData,
    updateRaceSkillsData, updateRaceDrawbackData, updateNameData,
} from "../Utility/UpdateUtility.js"

type Captured = { text: string, collapsed: boolean, depth: number }

const capture = (run: () => void) => {
    const out: Captured[] = []
    let depth = 0
    const g = vi.spyOn(console, "group").mockImplementation((...a: any[]) => { out.push({ text: a[0], collapsed: false, depth }); depth++ })
    const gc = vi.spyOn(console, "groupCollapsed").mockImplementation((...a: any[]) => { out.push({ text: a[0], collapsed: true, depth }); depth++ })
    const ge = vi.spyOn(console, "groupEnd").mockImplementation(() => { depth-- })
    const l = vi.spyOn(console, "log").mockImplementation((...a: any[]) => { out.push({ text: a[0], collapsed: false, depth }) })

    run()

    g.mockRestore(); gc.mockRestore(); ge.mockRestore(); l.mockRestore()
    expect(depth).toBe(0)
    return out
}

const show = (out: Captured[]) => out
