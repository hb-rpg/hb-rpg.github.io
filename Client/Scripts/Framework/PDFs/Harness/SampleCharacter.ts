...
function addFixed<T>(pkg: TaggedObservableSelectionPackage<T>, ...payloads: T[]): void {
    pkg.FixedSelection.push(...payloads.map(p => createTaggedData(standardSourceTag, p)))
}

const entanglement = (name: string, attitude: DispositionsEnum) =>
    new Entanglements({ id: 0, name }, attitude, 'Person')

export function makeSampleCharacter(): ConfiguredCharacterData {
    ...
    // ── Languages ────────────────────────────────────────────────────────────────
    addFixed(character.LanguageSelections(),
        new LearnedLanguage(new Language('Common', 'The trade tongue of the realm.', 100, 'Ch 4-1'), true, true, true),
        new LearnedLanguage(new Language('Old Dwarvish', 'Runic mountain speech.', 30, 'Ch 4-2'), true, true, false),
    )

    // ── Skills ───────────────────────────────────────────────────────────────────
    addFixed(character.SkillsSelection(),
        new Skill('Athletics', 'Climbing, swimming, and feats of raw strength.', 'Ch 3-2'),
        new Skill('Intimidation', 'Cow opponents through force of presence.', 'Ch 3-5'),
    )

    // ── Edges ────────────────────────────────────────────────────────────────────
    addFixed(character.EdgeSelections(),
        new Edges('Iron Grip', 'Cannot be disarmed by a single failed test.', 'Ch 5-1'),
        new Edges('Second Wind', 'Recover hit points once per encounter.', 'Ch 5-4'),
    )
    ...
    // ── Drawbacks ────────────────────────────────────────────────────────────────
    addFixed(character.DrawbacksSelection(),
        new Drawbacks('Hot-Headed', 'Temperament', 'Provoked easily; takes a penalty to resist taunts.', 'Ch 7-2'),
    )

    // ── Entanglements (attitudes shown in the Entanglements column) ───────────────
    character.OrganizationEntanglements(new OrganizationEntanglementsGroup(
        entanglement('Guild colleagues', DispositionsEnum.Friendly),
        entanglement('House Aldermoor', DispositionsEnum.Receptive),
        entanglement('City watch', DispositionsEnum.Disinterested),
        entanglement('Temple of the Dawn', DispositionsEnum.Negative),
        entanglement('Master-at-arms', DispositionsEnum.Friendly),
        entanglement('Riverside neighbors', DispositionsEnum.Receptive),
        entanglement('The Black Coins', DispositionsEnum.Hostile),
    ))
