...
const body = (data) => buildDMQuickReference(data)[0].table.body;
// Flatten each row's single cell to a plain string so tests can assert on what the DM reads.
const lines = (data) => body(data).map(([cell]) => Array.isArray(cell.text) ? cell.text.map((run) => run.text).join('') : String(cell.text));
const lineStartingWith = (data, prefix) => lines(data).find(line => line.startsWith(prefix));
const addFixed = (pkg, ...payloads) => pkg.FixedSelection.push(...payloads.map(p => createTaggedData(standardSourceTag, p)));
// Builds a character whose entire item list is `items`, with no trinkets unless given.
const characterWith = (items, trinkets = []) => {
    ...
    data.TrinketSelections(TaggedObservableSelectionPackageFactory(new SelectionPackage(trinkets, [], []))());
    return data;
};
