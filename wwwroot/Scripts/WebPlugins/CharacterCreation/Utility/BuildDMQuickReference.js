...
const CARD_LABEL = [true, true, true, true];
const CARD_MIDDLE = [true, false, true, false];
const CARD_BOTTOM = [true, false, true, true];
...
const COMBAT_TYPES = [ItemTypes.Melee, ItemTypes.Ranged, ItemTypes.Ammo, ItemTypes.Armor];
const isCombatItem = (item) => item.Type !== undefined && COMBAT_TYPES.includes(item.Type);
...
const QUANTIFIED_TYPES = [ItemTypes.Ration, ItemTypes.Rope, ItemTypes.Container];
// "Rations (4 servings)", "Wineskin ×2". Amount is 1 for most items and would be noise everywhere.
function gearEntry(item) {
...
function abilityLine(abilities) {
...
const namedEntries = (entries) => entries.map(entry => ({
    text: [
...
export function buildDMQuickReference(data) {
    ...
    const spellBlock = spells.length > 0
